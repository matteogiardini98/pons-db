...
const DashboardView = () => {
  ...
  const euComplianceData = [{
    name: 'GDPR Compliant',
    value: metrics.gdprCompliant
  }, {
    name: 'EU Data Residency',
    value: metrics.euDataResidency
  }];
  ...
  return <div className="min-h-screen pt-20">
      <div className="container-tight py-8">
        <div className="mb-8">
          <h2 className="text-3xl font-display font-medium mb-4">pons dash</h2>
          <p className="text-muted-foreground max-w-2xl">gain insights into AI solutions across industries, functions, and technical requirements.</p>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          <StatsCard title="Total AI Tools" value={metrics.totalTools} description="Solutions in database" />
          <StatsCard title="GDPR Compliant" value={`${Math.round(metrics.gdprCompliant / metrics.totalTools * 100)}%`} description="Of tools are GDPR compliant" />
          <StatsCard title="EU Data Residency" value={`${Math.round(metrics.euDataResidency / metrics.totalTools * 100)}%`} description="Store data within EU borders" />
        </div>
              ...
              <CardContent>
                <div className="h-[350px]">
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={industryData} layout="vertical" margin={{
                    top: 20,
                    right: 30,
                    left: 60,
                    bottom: 5
                      ...
                      <Tooltip formatter={(value: any) => [`${value} tools`, 'Count']} contentStyle={{
                      background: 'white',
          ...
          <TabsContent value="functions" className="space-y-6">
            <Card>
              ...
              <CardContent>
                <div className="h-[350px]">
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={functionData} layout="vertical" margin={{
                    top: 20,
                    right: 30,
                    left: 60,
                    bottom: 5
            ...
            <Card>
              ...
              <CardContent className="flex justify-center">
                <div className="h-[350px] w-full">
                  <ResponsiveContainer width="100%" height="100%">
                    <PieChart>
                      <Pie data={businessTypeData} cx="50%" cy="50%" labelLine={true} label={({
                      name,
                      percent
                    }) => `${name}: ${(percent * 100).toFixed(0)}%`} outerRadius={130} fill="#8884d8" dataKey="value">
                        {businessTypeData.map((entry, index) => <Cell key={`cell-${index}`} fill={COLORS[index % COLORS.length]} />)}
                      </Pie>
                      <Tooltip formatter={(value: any) => [`${value} tools`, 'Count']} contentStyle={{
                      background: 'white',
                      border: '1px solid #e2e8f0',
            ...
            <Card>
              ...
              <CardContent className="flex justify-center">
                <div className="h-[350px] w-full">
                  <ResponsiveContainer width="100%" height="100%">
                    <PieChart>
                      <Pie data={technicalLevelData} cx="50%" cy="50%" labelLine={true} label={({
                      name,
                      percent
                    }) => `${name}: ${(percent * 100).toFixed(0)}%`} outerRadius={130} fill="#8884d8" dataKey="value">
                        {technicalLevelData.map((entry, index) => <Cell key={`cell-${index}`} fill={COLORS[index % COLORS.length]} />)}
                      </Pie>
                      <Tooltip formatter={(value: any) => [`${value} tools`, 'Count']} contentStyle={{
                      background: 'white',
                      border: '1px solid #e2e8f0',
          ...
          <TabsContent value="eu-compliance" className="space-y-6">
            <Card>
              ...
              <CardContent>
                <div className="h-[350px]">
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={euComplianceData} margin={{
                    top: 20,
                    right: 30,
                    left: 20,
                    bottom: 5
